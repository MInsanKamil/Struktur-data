    ...
    def printHelper(self,node):
        if node:
            ...
            if type(node.getKey()) == str and type(node.getValue()) == str:
                if node == self.minKeyNode(self.root) and node.getRight() == self.nil and node.getLeft() == self.nil and node == self.root:
                    txt3 = "{}'{}': '{}'".format("{",node.getKey(),node.getValue())
                    print(txt3, end="}\n")
                elif node == self.minKeyNode(self.root):
                    txt3 = "{}'{}': '{}'".format("{",node.getKey(),node.getValue())
                    print(txt3, end=", ")
                elif node == self.maxKeyNode(self.root):
                    print(f"'{node.getKey()}': '{node.getValue()}'", end="}\n")
                else:
                    print(f"'{node.getKey()}': '{node.getValue()}'", end=", ")
            elif type(node.getKey()) == str and type(node.getValue()) != str:
                if node == self.minKeyNode(self.root):
                    txt3 = "{}'{}': {}".format("{",node.getKey(),node.getValue())
                    print(txt3, end=", ")
                elif node == self.maxKeyNode(self.root):
                    print(f"'{node.getKey()}': {node.getValue()}", end="}\n")
