...
class RBTree:
    ...
    def exist(self, key):
        if self.root == None:
            print(f"Tree Kosong, node dengan key:{key} Tidak Ada")
        elif self.search(key):
            print(f"Node dengan key:{key} Ada")
            return True
        else:
            print(f"Node dengan key:{key} Tidak Ada")
            return False
    ...
    def edit(self, key, value):
        if self.exist(key):
            ...
            print(f"Node dengan key:{key}, valuenya telah diubah dari {temp} menjadi {self.search(key).getValue()}")
        else:
